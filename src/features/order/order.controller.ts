...
export class OrderController {
  ...
  async capturePaymentPaypal(@Req() req: Request, @Query('token') token: string):Promise<any> {
    const user = req['user']
    if(!user){
      throw new HttpException(" Unauthorized", HttpStatus.UNAUTHORIZED)
    }
  ...
  @HttpCode(HttpStatus.OK)
  async verifyPaymentPaypal(@Req() req: Request, @Query('token') token: string):Promise<any> {
    const user = req['user']
    if(!user){
      throw new HttpException(" Unauthorized", HttpStatus.UNAUTHORIZED)
    }
  ...
  @HttpCode(HttpStatus.OK)
  async verifyPaymentCOD(@Req() req: Request, @Param('id') orderId: string):Promise<any> {
    const user = req['user']
    if(!user){
      throw new HttpException(" Unauthorized", HttpStatus.UNAUTHORIZED)
    }
